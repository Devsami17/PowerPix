...
function updateVisibleCards() {
  const totalVisible = currentPage * cardsPerPage;
  cards.forEach((card, index) => {
    card.style.display = index < totalVisible ? 'block' : 'none';
  });
  if (totalVisible >= cards.length) {
...
function checkYourNumbers(button){
  button.textContent = "Checking...";
  setTimeout(() => {
    document.querySelectorAll('.check-your-numbers-card').forEach((item)=>item.classList.toggle('active'))
    button.textContent = "Search";
  }, 3000);

}
function clearCheckNumbers(){
